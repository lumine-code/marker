  ...
  constructor({ registry, className, emptyMessage, disabledKey, extras = [] }) {
    ...
    this.selectList = lumine.workspace.buildSelectList({
      ...
      elementForItem: (item, { highlight }) => {
        // Return a descriptor so the select list creates the row in the modal's
        // own Document. `didRender` then decorates that realm-local element.
        return {
          primary: "",
          didRender: (li) => {
            const document = li.ownerDocument;
            const primary = li.querySelector(".primary-line");
