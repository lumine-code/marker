    ...
    return {
      ...
      renderer: {
        ...
        row: (item, ctx) => {
          ...
          //
          // An element, never a DocumentFragment: the kernel caches the Row and
          // repaints from it, and appending a fragment empties it, so every
          // label would come back blank on the second render.
          const label = document.createElement("span");
          const tag = document.createElement("span");
          tag.classList.add("tag");
          tag.appendChild(atom.modals.ui.highlight(item.name, ctx.highlights.label));
