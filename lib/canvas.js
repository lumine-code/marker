...
class MarkerStyles {
   ...
   * @param {Document} options.document Document that owns the renderer.
  ...
  constructor({
    ...
    if (!document?.defaultView) {
      throw new TypeError("MarkerStyles requires a live Document");
    }
  ...
  probeFor(className) {
    ...
    if (!probe) {
      probe = this.element.ownerDocument.createElement("div");
...
class MarkerCanvas {
   ...
   */
  constructor({ document, className, canvasClass, resolverClass, probeClass }) {
    if (!document?.defaultView) {
      throw new TypeError("MarkerCanvas requires a live Document");
    }
